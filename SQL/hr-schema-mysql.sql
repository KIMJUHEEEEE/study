...
SELECT employee_id 사원번호, first_name 이름, salary 연봉
FROM employees
...
SELECT first_name 이름, department_id 부서번호, salary 연봉
FROM employees 
WHERE hire_date='1996-07-18';
...
SELECT first_name, salary, salary*commission_pct, salary+NVL((salary*commission_pct),0) 총액
FROM employees 
...
SELECT ROUND(salary/12/5)
FROM employees
WHERE department_id =20;

SELECT department_id, first_name,hire_date,sysdate,sysdate-hire_date,to_char(sysdate,'YYYY')-to_char(hire_date,'YYYY'),(sysdate-hire_date)/30
FROM employees;
...
SELECT avg(salary),MAX(salary),MIN(salary),count(department_id)
FROM employees
...
SELECT CONCAT(e.first_name,'의 매니저는 ',e2.first_name,'이다.') 매니저정보출력
...
SELECT department_id, first_name, hire_date, CURDATE(), TIMESTAMPDIFF(DAY,hire_date,CURDATE()),TIMESTAMPDIFF(YEAR,hire_date,CURDATE()), ROUND(TIMESTAMPDIFF(DAY,hire_date,CURDATE())/30)
FROM employees;

SELECT EXTRACT(YEAR FROM hire_date)
FROM employees;

SELECT DATE_FORMAT(hire_date,'%Y"년" %c"월" %e"일"')
FROM employees;
